...
After we parallel transport from $A$ to $B$ along a null ray, we can again parallel transport the resulting vector in $B$ along another curve back to $A$.
If we compare the final vector back at $A$ with the initial vector, the result is a loop integral.
\begin{equation}
\Delta e^c = \int_A^B \hat{k}^a e_0^b \Gamma_{ab}^c~dl +
