...
\begin{abstract}
From the basic concept of general relativity, we derive how the polarization of light can be rotated by a moving gravitational lens. 
...
It was summarized in \cite{BroDem11} that three different values of $\Delta\phi$ can be derived from existing literature for identical physical situation.
...
In Sec.\ref{sec-born}, we will provide the operational definition of polarization from the basic principles in general relativity and explain the inevitable gauge dependence.
In Sec.\ref{sec-Sch}, we will derive Eq.~(\ref{eq-v}) in the small rotation limit for one point-mass lens, and show that it can be generalized into multiple lenses.
...
If we further assume that the masses are distributed in a small enough region, the answer is very simple. 
\begin{equation}
...
\label{eq-spin}
\end{eqnarray}
The spin of the point mass contributes in exactly the same way as its ``orbital'' angular momentum around the light ray. Intriguingly, although \cite{KopMas01} agrees with Eq.~(\ref{eq-gen}), they claimed that the spin does not contribute at all. We cannot see any physical reason for such disagreement since Eq.~(\ref{eq-spin}) seems to be the most natural result.
